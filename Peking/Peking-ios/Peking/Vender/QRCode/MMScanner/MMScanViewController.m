...
- (void)viewDidLoad {
    ...
    [self initNavigationRightButtonWithTitle:LAN(@"相册") titileColor:kNVABICSYSTEMTitleColor];
...
- (void)initScanType {
    if (self.scanType == MMScanTypeAll) {
        ...
        _scanRect = CGRectFromString([self scanRectWithScale:1][0]);
        self.output.rectOfInterest = _scanRect;
        _tipTitle.text = LAN(@"将取景框对准二维码,即可自动扫描");
        ...
        _tipTitle.text = LAN(@"将取景框对准条码,即可自动扫描");
...
- (void)drawBottomItems
{
    ...
    [_scanTypeQrBtn setTitle:LAN(@"二维码") forState:UIControlStateNormal];
    ...
    [_scanTypeBarBtn setTitle:LAN(@"条形码") forState:UIControlStateNormal];
...
- (void)drawFlashBtn {
    ...
    [_flashBtn setTitle:LAN(@"轻触照亮") forState:UIControlStateNormal];
    [_flashBtn setTitle:LAN(@"轻触关闭") forState:UIControlStateSelected];
...
- (void)setNavItem:(MMScanType)type {
    ...
    if(type == MMScanTypeBarCode) {
        ...
        if (_historyCallBack) {
            _photoItem = [[UIBarButtonItem alloc] initWithTitle:LAN(@"相册") style:UIBarButtonItemStylePlain target:self action:@selector(openPhoto)];
            _historyItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"scan_history" inBundle:scanBundle compatibleWithTraitCollection:nil] style:UIBarButtonItemStylePlain target:self action:@selector(callbackHistory)];
...
- (void)changeScanCodeType:(MMScanType)type {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        _tipTitle.text = type == MMScanTypeQrCode ? LAN(@"将取景框对准二维码,即可自动扫描") : LAN(@"将取景框对准条码,即可自动扫描");
...
- (void)openPhoto
{
    ...
    {
        NSString *tipMessage = [NSString stringWithFormat:LAN(@"请到手机系统的\n【设置】->【隐私】->【相册】\n对\"%@\"开启相机的访问权限"),appName];
        [self showError:tipMessage andTitle:LAN(@"相册读取权限未开启")];
    }
}
...
- (BOOL)isAvailableCamera {
    if([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])
    {
        ...
        if (authorizationStatus == AVAuthorizationStatusRestricted ||
            ...
            NSString *tipMessage = [NSString stringWithFormat:LAN(@"请到手机系统的\n【设置】->【隐私】->【相机】\n对\"%@\"开启相机的访问权限"),appName];
            [self showError:tipMessage andTitle:LAN(@"相机权限未开启")];
...
- (void)showError:(NSString*)str {
    [self showError:str andTitle:LAN(@"提示")];
}
...
- (void)showError:(NSString*)str andTitle:(NSString *)title
{
    ...
    UIAlertAction *action = ({
        UIAlertAction *action = [UIAlertAction actionWithTitle:LAN(@"知道了") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)recognizeQrCodeImage:(UIImage *)image onFinish:(void (^)(NSString *result))finish {
    ...
    if ([[[UIDevice currentDevice]systemVersion]floatValue] < 8.0 ) {
        ...
        [self showError:LAN(@"只支持iOS8.0以上系统")];
        return;
    }
