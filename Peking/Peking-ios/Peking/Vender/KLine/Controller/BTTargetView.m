...
- (void)initialize
{
    self.backgroundColor = kBICWhiteColor;//R:0.05 G:0.09 B:0.14 A:1
    self.array = [NSMutableArray array];
    [self.array addObject:@[@"MA",@"EMA",@"BOLL"]];
//        [self.array addObject:@[@"MA"]];
...
-(UIView *)headerView_0
{
    if (!_headerView_0) {
        ...
        label.textColor = kBICSYSTEMBGColor;
...
//    [cell.titleBtn setTitle:title forState:UIControlStateSelected];
    cell.titleBtn.text=title;
    if (indexPath.section == 0) {
        if (indexPath.row == self.selectRow_0) {
//            [cell.titleBtn setSelected:YES];
            cell.titleBtn.textColor=kBICSYSTEMBGColor;
        }else{
//            [cell.titleBtn setSelected:NO];
            cell.titleBtn.textColor=kBICGetHomeCHRColor;
        }
...
//        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        ...
        btn.font=FONT_WITH_SIZE(10);
        btn.textAlignment=NSTextAlignmentCenter;
//        btn.titleLabel.font = FONT_WITH_SIZE(10);
        [self addSubview:btn];
        self.titleBtn = btn;
        self.selectionStyle=UITableViewCellSelectionStyleNone;
        btn.textColor=kBICGetHomeCHRColor;
//        [btn setTitleColor:kBICGetHomeCHRColor forState:UIControlStateNormal];
//        [btn setTitleColor:kBICSYSTEMBGColor forState:UIControlStateSelected];
//        btn.userInteractionEnabled = NO;
//        [btn addTarget:self action:@selector(sysLog) forControlEvents:UIControlEventTouchUpInside];
        [btn mas_makeConstraints:^(MASConstraintMaker *make) {
