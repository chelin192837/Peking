...
//    _closeLabel.text = [CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:model.Close.doubleValue  afterPoint:model.price];
     if(!self.type){
        ...
        _MA30Label.text = [NSString stringWithFormat:@" MA30：%@",[CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:model.MA30.doubleValue  afterPoint:model.coin]];
          ...
          [self maProfileWithModel:model type:self.type];
     }
...
-(void)maProfileWithModel:(Y_KLineModel *)model type:(NSInteger)type
{
    ...
    NSString *dateStr = [formatter stringFromDate:date];
    _dateDescLabel.text = [@" " stringByAppendingString: dateStr];
    if(type==3001 || type==Y_StockChartTargetLineStatusMA){
        _MA7Label.text = [NSString stringWithFormat:@"  MA7:%@",[CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:(double)model.Volume_MA7.doubleValue afterPoint:model.coin]];
        _MA30Label.text = [NSString stringWithFormat:@"  MA30:%@",[CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:(double)model.Volume_MA30.doubleValue afterPoint:model.coin]];
    }else if(type==3002 || type==Y_StockChartTargetLineStatusEMA){
        _MA7Label.text = [NSString stringWithFormat:@"  EMA7:%@",[CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:(double)model.EMA7.doubleValue afterPoint:model.coin]];
        _MA30Label.text = [NSString stringWithFormat:@"  EMA30:%@",[CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:(double)model.EMA30.doubleValue afterPoint:model.coin]];
    }else if(type==3003 || type==Y_StockChartTargetLineStatusBOLL){
        _MA7Label.text = [NSString stringWithFormat:@"  BOLL_MD:%@",[CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:(double)model.BOLL_MD.doubleValue afterPoint:model.coin]];
        _MA30Label.text = [NSString stringWithFormat:@"  BOLL_MB:%@",[CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:(double)model.BOLL_MB.doubleValue afterPoint:model.coin]];
    }
