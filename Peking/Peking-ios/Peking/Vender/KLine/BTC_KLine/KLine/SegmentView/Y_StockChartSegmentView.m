...
- (void)setItems:(NSArray *)items
{
    ...
    if (!self.isFullScreen) {
        ...
        line.backgroundColor = kBICWhiteColor;
    ...
    for (int i=0;i<items.count;i++)
    {
        ...
        view.backgroundColor = kBICWhiteColor;
        [self addSubview:btn];
        [self addSubview:view];
        [btn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self);
            if(i==items.count-1){
                make.width.equalTo(self).multipliedBy(1.5f/count);
...
- (UIButton *)private_createButtonWithTitle:(NSString *)title tag:(NSInteger)tag
{
    ...
    [btn setTitleColor:kBICGetHomeCellPriceColor forState:UIControlStateNormal];
    ...
    if ([title isEqualToString:LAN(@"1月")]) {
        btn.hidden = YES;
    }
...
- (void)event_segmentButtonClicked:(UIButton *)btn {
    ...
    //发送按钮点击时间
    [[NSNotificationCenter defaultCenter] postNotificationName:NSNotificationCenteSockJSReloadFlag object:@(btn.tag-Y_StockChartSegmentStartTag)];
