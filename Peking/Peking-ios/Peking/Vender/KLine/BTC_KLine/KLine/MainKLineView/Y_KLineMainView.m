...
//        if(model.OpenPoint.x<self.parentScrollView.frame.size.width){
//            self.oldContentOffsetX=0;
//        }
        // 绘制蜡烛线
...
- (void)drawBottomDateValue {
    ...
    [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(Y_KLinePositionModel * _Nonnull positionModel, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.lineKTime == 0 || self.lineKTime == 1 ) {
            formatter.dateFormat = @"HH:mm";
        }
        else if (self.lineKTime == 6 || self.lineKTime == 7 || self.lineKTime == 8 || self.lineKTime == 9 || self.lineKTime == 10 ){
            formatter.dateFormat = @"YYYY MM-dd";
        }
        else if (self.lineKTime == 2 || self.lineKTime == 3 || self.lineKTime == 4 || self.lineKTime == 5) {
...
- (void)drawMaxAndMinValueAndCurrentPrice{
    ...
    //fix crash
    CGPoint pointYY = CGPointMake(0, 0);
    CGFloat MinX = pointYY.x;
    if ((self.needDrawKLinePositionModels.count>lowIndex) && (self.needDrawKLinePositionModels)) {
        Y_KLinePositionModel *lowPositionModel = self.needDrawKLinePositionModels[lowIndex];
        pointYY = lowPositionModel.LowPoint;
        MinX = pointYY.x;
    }
    //
...
//        y = ABS(maxY - (model.Close.doubleValue - low.doubleValue)/unitValue);
    ...
    //横屏实时价格的x坐标 减50（指标宽度）--77--
    [self.currentPriceView setFrame:CGRectMake(self.isFullScreen?(priceRectx-50-SafeAreaBottomHeight-(is_iPhoneX?34:0)):priceRectx - 30-30, y - 7.5, Y_StockChartKLinePriceViewWidth+30, 15)];
...
//    NSAssert(self.kLineModels, @"kLineModels不能为空");
    RSDLog(@"kLineModels不能为空%@",self.kLineModels);
    //提取需要的kLineModel
