...
- (void)controllerWillDisAppear {
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"CWCarouselFirst"];
    if(self.isAuto) {
        [self pause];
    }
    [self adjustErrorCell:YES];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@YES forKey:@"CWCarouselFirst"];
    [UIView animateWithDuration:0.5 animations:^{
        UICollectionViewCell *cell = [self.carouselView cellForItemAtIndexPath:self.currentIndexPath];
...
- (void)setCustomPageControl:(UIView<CWCarouselPageControlProtocol> *)customPageControl {
    ...
    if(_customPageControl && _customPageControl.superview == nil)
    {
        ...
        [_customPageControl mas_makeConstraints:^(MASConstraintMaker *make) {
