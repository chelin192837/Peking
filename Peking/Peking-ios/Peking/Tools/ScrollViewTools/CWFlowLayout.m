...
- (void)prepareLayout {
    switch (self.style) {
        ...
        case CWCarouselStyle_H_2: {
            self.itemWidth = 290;
            self.itemSize = CGSizeMake(290,  380);
            self.scrollDirection = UICollectionViewScrollDirectionHorizontal;
            self.factItemSpace = 10;
       ...
       self.style != CWCarouselStyle_H_1) {
        ...
        [arr enumerateObjectsUsingBlock:^(UICollectionViewLayoutAttributes * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if(space > 0) {
                ...
                if (![[NSUserDefaults standardUserDefaults] objectForKey:@"CWCarouselFirst"]) {
...
- (CGFloat)defaultItemWidth {
    switch (self.style) {
        ...
        case CWCarouselStyle_H_2:
        case CWCarouselStyle_H_3:
            return self.collectionView.frame.size.width * 0.725;
            break;
        default:
            break;
    }
