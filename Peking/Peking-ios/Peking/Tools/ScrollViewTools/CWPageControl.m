...
- (void)createIndicator {
    ...
    for(int i = 0; i < self.pageNumbers; i ++) {
        ...
        indictor.userInteractionEnabled = YES;
        indictor.layer.cornerRadius = height/2;
        [container addSubview:indictor];
        indictor.backgroundColor = [UIColor colorFromHexRGB:@"dddddd"];
        [arr addObject:indictor];
        if(i == 0) {
            self.currentIndicator = [[UIView alloc] initWithFrame:CGRectMake(x, y, width, height)];
            self.currentIndicator.userInteractionEnabled = YES;
            self.currentIndicator.layer.cornerRadius = height/2;
            [container addSubview:self.currentIndicator];
            self.currentIndicator.backgroundColor = DefaultColorOfGreen;
