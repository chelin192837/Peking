...
//    [self.btn setTitle:@"打开七扇门" forState:UIControlStateNormal];
    [self.btn addTarget:self action:@selector(removeViewBtn:) forControlEvents:UIControlEventTouchUpInside];
    
//    self.nextButton = [UIButton buttonWithType:UIButtonTypeCustom];
//    self.nextButton.frame = CGRectMake(0, 0, 100, 100);
//    self.nextButton.center = CGPointMake(self.frame.size.width/2, self.pageV.frame.origin.y - self.btn.frame.size.height/2);
//    self.nextButton.hidden = NO;
//    self.nextButton.layer.masksToBounds = YES;
//    self.nextButton.layer.cornerRadius = 50;
//    [self.nextButton addTarget:self action:@selector(nextBtnClick:) forControlEvents:UIControlEventTouchUpInside];
//    [self.nextButton setImage:[UIImage imageNamed:@"news_xitong_icon"] forState:UIControlStateNormal];
...
//    self.pageV.currentPage = self.pageV.currentPage +1;
//    [_collectionV setContentOffset:CGPointMake(self.pageV.currentPage*self.frame.size.width , 0) animated:YES];
//}
