...
@implementation BICMineNewCell

+(instancetype)cellWithTableView:(UITableView *)tableView{
    static NSString *reuseIdentifier = @"BICMineNewCell";
    BICMineNewCell *cell=[tableView dequeueReusableCellWithIdentifier:reuseIdentifier];
    if(!cell){
...
-(void)layoutSubviews{
    ...
    self.titleTexLab.frame=CGRectMake(CGRectGetMaxX(self.titleImg.frame)+12, 0, 200, 56);
...
-(void)setResponse:(BICAuthInfoResponse *)response{
    _response=response;
    if([self.response.data.status isEndWithString:@"W"]){
