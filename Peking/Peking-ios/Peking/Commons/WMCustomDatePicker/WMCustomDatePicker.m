...
#define DATE_GRAY [UIColor lightGrayColor];
...
{
    //  初始化数组
    yearNowArray   = [self ishave:yearNowArray];
    ...
    for (int i = DATEPICKER_MINDATE; i<= yearNow; i++) {
        [yearNowArray addObject:[NSString stringWithFormat:@"%d年",i]];
    }
    [yearNowArray addObject:@"至今"];
    ...
    myPickerView = nil;
    myPickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, 216)];
...
- (NSArray *)getNowDate:(NSDate *)date
{
    ...
    yearNowIndex = [model.year intValue]-DATEPICKER_MINDATE + 1; //???
    ...
    NSNumber * yearNow = [NSNumber numberWithInteger:yearNowIndex];
    ...
    if (self.datePickerStyle == WMDateStyle_YearMonthNow) //????
        return @[yearNow,month];
    return nil;
}
//武猛添加
- (NSInteger)findMinuteIndex:(NSString *)minute
{
    for ( int i = 0; i<60; i++) {
        ...
        {
            minute = [NSString stringWithFormat:@"%d",[minute integerValue] +1] ;
        }
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    ...
    {
        if (component == 0) return DATEPICKER_MAXDATE-DATEPICKER_MINDATE;
        else                return DATEPICKER_MONTH;
    }
    if (self.datePickerStyle == WMDateStyle_YearMonthNow)
    {
        if (component == 0) return yearNow-DATEPICKER_MINDATE + 1+1;
...
- (void)playTheDelegate
{
    ...
    if ([self.delegate respondsToSelector:@selector(finishDidSelectDatePicker:year:month:day:hour:minute:weekDay:)]) {
        NSString * year ;
        if (self.datePickerStyle == WMDateStyle_YearMonthNow) {
            if ([yearNowArray[yearNowIndex] isEqualToString:@"至今"]) {
                year = @"至今";
            }else
            {
                year = yearNowArray[yearNowIndex];
            }
        }else
        {
