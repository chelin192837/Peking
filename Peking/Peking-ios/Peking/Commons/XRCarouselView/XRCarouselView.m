...
//分页控件
@property (nonatomic, strong) XRPageControl *pageControl;
//当前显示的imageView
...
- (XRPageControl *)pageControl {
    if (!_pageControl) {
        _pageControl = [[XRPageControl alloc] init];
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            else [_images addObject:kBuildingLongImage_Placeholder];
...
- (void)nextPage {
    if (_changeMode == ChangeModeFade) {
        ...
        [UIView animateWithDuration:1.2 animations:^{
            ...
            self.pageControl.currentPage = weakSelf.nextIndex;
