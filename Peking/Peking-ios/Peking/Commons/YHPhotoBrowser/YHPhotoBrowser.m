...
- (void)setupToolbars
{
    ...
    _indexLabel.frame =  CGRectMake(10, self.bounds.size.height - 33, 60, 30);
    _saveButton.frame = CGRectMake(kScreenWidth-70, self.bounds.size.height - 33, 60, 30);
}
...
-(void)saveImage
{
    ...
    //gif 图片
    if ([currImageView.mainUrl.absoluteString hasSuffix:@".gif"]) {
        
        [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:currImageView.mainUrl options:SDWebImageDownloaderUseNSURLCache progress:nil completed:^(UIImage *image, NSData *data, NSError *error, BOOL finished) {
...
-(void)setUrlImgArr:(NSArray *)urlImgArr{
    
    _urlImgArr=urlImgArr;
    _calcIndex=0;
    [self initScrollContSize];
    _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", _calcIndex+1,(long)self.urlImgArr.count];
...
-(void)ifNeedScroll:(NSInteger)tag{
    ...
    if((tag>0&&tag<=_urlImgArr.count-2)||(_urlImgArr.count==2&&tag==1))
    {
        ...
        _calcIndex=tag-1;
        _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", _calcIndex+2,(long)self.urlImgArr.count];
    ...
    }else if(tag==_urlImgArr.count-1&&_urlImgArr.count>1)// 最后一个图 需要滚动到最后
    {
        ...
        _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", _calcIndex+3,(long)self.urlImgArr.count];
    }
    else{
        _calcIndex=tag;
        _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", _calcIndex+1,(long)self.urlImgArr.count];
...
-(void)openImgAnimayion{
    ...
    if(CGRectIsEmpty(_sourceRect)||_sourceView==nil){
        ...
        if ([_urlImgArr[_indexTag] isKindOfClass:[NSString class]]) {
            [tempView yy_setImageWithURL:[NSURL URLWithString:_urlImgArr[_indexTag]] placeholder:nil];
        }else{
            [tempView setImage:_urlImgArr[_indexTag]];
        }
...
- (void)updateSubImageView:(NSInteger)pageIndex with:(NSInteger)index {
    ...
    if(index==0){
        if ([_urlImgArr[_indexTag] isKindOfClass:[NSString class]]) {
            ...
            [_leftBrowserImgView setImageWithImage:_urlImgArr[pageIndex] placeholderImage:nil];
        }
        ...
        if ([_urlImgArr[_indexTag] isKindOfClass:[NSString class]]) {
            ...
            [_midBrowserImgView setImageWithImage:_urlImgArr[pageIndex] placeholderImage:nil];
        }
        ...
        if ([_urlImgArr[_indexTag] isKindOfClass:[NSString class]]) {
            ...
            [_rightBrowserImgView setImageWithImage:_urlImgArr[pageIndex] placeholderImage:nil];
        }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", self.curentIndex+1,(long)self.urlImgArr.count];
}
...
- (void)setHidenOption:(BOOL)hidenOption {
    
    _hidenOption = hidenOption;
